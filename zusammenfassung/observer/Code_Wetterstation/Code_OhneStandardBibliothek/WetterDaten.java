package Kapitel.Kapitel2_ObserverMuster.Code_Wetterstation.Code_OhneStandardBibliothek;

import java.util.ArrayList;

/**
 *
 * @author silas
 */
public class WetterDaten implements Subjekt {

    private ArrayList beobachter;
    private float temperatur;
    private float feuchtigkeit;
    private float luftdruck;
    private float waermeIndex;

    public WetterDaten() {
        this.beobachter = new ArrayList();
    }

    @Override
    public void registriereBeobachter(Beobachter b) {
        this.beobachter.add(b);
    }

    @Override
    public void entferneBeobachter(Beobachter b) {
        this.beobachter.remove(b);
    }

    @Override
    public void benachrichtigeBeobachter() {
        for (int i = 0; i < this.beobachter.size(); i++) {
            Beobachter b = (Beobachter)this.beobachter.get(i);
            b.aktualisieren(this.temperatur, this.feuchtigkeit, this.luftdruck, this.waermeIndex);
        }
    }
    
    public void messwerteGeaendert() {
        benachrichtigeBeobachter();
    }
    
    public void setMesswerte(float temp, float feucht, float druck) {
        this.temperatur = temp; 
        this.feuchtigkeit = feucht; 
        this.luftdruck = druck; 
        this.waermeIndex = wärmeIndex(temp, feucht);
        messwerteGeaendert();
    }
    
     private float wärmeIndex(float t, float rh) {
        float index = (float)((16.923 + (0.185212 * t) + (5.37941 * rh) - (0.100254 * t * rh) +
            (0.00941695 * (t * t)) + (0.00728898 * (rh * rh)) +
            (0.000345372 * (t * t * rh)) - (0.000814971 * (t * rh * rh)) +
            (0.0000102102 * (t * t * rh * rh)) - (0.000038646 * (t * t * t)) + (0.0000291583 *  
            (rh * rh * rh)) + (0.00000142721 * (t * t * t * rh)) +
            (0.000000197483 * (t * rh * rh * rh)) - (0.0000000218429 * (t * t * t * rh * rh)) +     
            0.000000000843296 * (t * t * rh * rh * rh)) -
            (0.0000000000481975 * (t * t * t * rh * rh * rh)));
        return index;
    }

}
